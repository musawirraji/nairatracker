...
function StatCard({
  ...
  return (
    <Card style={{ padding: T.s.lg }}>
      ...
      <div style={{ fontSize: T.f.xl, fontWeight: 800, color }}>
        {fmt(value)}
      </div>
      {sub && (
        <div style={{ fontSize: 10, color: T.color.dim, marginTop: 4 }}>
          {sub}
        </div>
      )}
    </Card>
...
function BreakdownList({
  ...
  items: ReturnType<typeof transactionService.categoryBreakdown>;
  color: string;
}) {
  return (
...
export function Dashboard({ txns, goal, loading }: Props) {
  ...
  const expBreak = transactionService.categoryBreakdown(
    txns,
    'out',
    CATEGORY_COLORS,
  );
  const incBreak = transactionService.categoryBreakdown(
    txns,
    'in',
    CATEGORY_COLORS,
  );
  ...
  return (
    ...
    >
      ...
      <Card
        accent
        style={{
          background: 'linear-gradient(135deg,#111108 0%,#0A0A18 100%)',
          position: 'relative',
          overflow: 'hidden',
        }}
      >
        <div
          style={{
            ...
            height: 200,
            background:
              'radial-gradient(circle,rgba(255,208,50,0.12) 0%,transparent 65%)',
            borderRadius: '50%',
            pointerEvents: 'none',
          }}
        />
        ...
        <div
          style={{ fontSize: T.f.sm, color: T.color.soft, marginBottom: 20 }}
        >
          net saved of {fmt(goal)} target
        </div>
        ...
        >
          <div
            style={{
              width: `${progress}%`,
              height: '100%',
              background: `linear-gradient(90deg,${T.color.gold},#FF9800)`,
              borderRadius: 6,
              boxShadow: `0 0 16px rgba(255,208,50,0.5)`,
        ...
        >
          <span style={{ color: T.color.gold, fontWeight: 700 }}>
            {progress.toFixed(2)}%
          </span>
          <span style={{ color: T.color.soft }}>
            {fmt(goal - net)} remaining
          </span>
        </div>
        ...
        <div
          style={{
            ...
            borderRadius: T.r.md,
            border: '1px solid rgba(255,208,50,0.12)',
          }}
        >
          <div
            style={{
          ...
          <div
            style={{
              fontSize: T.f['2xl'],
              fontWeight: 800,
              color: onTrack ? T.color.green : T.color.gold,
            }}
          >
            {fmt(projected)}
          </div>
          <div
            style={{
              fontSize: T.f.xs,
              marginTop: 4,
              color: onTrack ? T.color.green : T.color.red,
            }}
          >
            {onTrack
              ? '✓ On track to hit your goal'
              : `${fmt(gap)} gap — push harder`}
          </div>
        </div>
      </Card>
      ...
      <Card>
        ...
        >
          <div>
            <div
              style={{
                fontSize: T.f['2xl'],
                fontWeight: 800,
                color: mthNet >= 0 ? T.color.green : T.color.red,
              }}
            >
              {mthNet >= 0 ? '+' : ''}
              {fmt(mthNet)}
            ...
            <div
              style={{ fontSize: T.f.xs, color: T.color.soft, marginTop: 3 }}
            >
              net this month
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            ...
            >
              +{fmt(transactionService.totalIn(mthTxns))}
            </div>
            <div
              style={{ fontSize: T.f.sm, fontWeight: 700, color: T.color.red }}
            >
              −{fmt(transactionService.totalOut(mthTxns))}
            </div>
          </div>
        </div>
      </Card>
